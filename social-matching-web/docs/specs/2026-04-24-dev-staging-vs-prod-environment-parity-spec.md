...
**פרומוט מעשי (צ׳קליסט + שער אוטומטי):** [`docs/ops/promote-staging-to-prod.md`](../ops/promote-staging-to-prod.md) — כולל `npm run ops:pre-promote-prod` לפני דיפלוי פרוד.
...
### 12יא) מוצר / עותק UI — למה רואים טקסטים שונים בין סטייג׳ לפרוד

**עקרון:** אותו **קומיט** שמפורסם לשני הדומיינים יפיק **אותו** bundle (אותם מחרוזות מ־`src/locales/he.ts`, מחרוזות inline ב־TSX, וכו’). ההבדלים שתיארת נובעים בדרך כלל מאחד מאלה:
...
| כיוון | כלי / פעולה |
|--------|-------------|
| שרירות בין דומיינים | `npm run ops:compare-staging-prod-spa-bundles` + השוואת קומיט ב־Vercel (Deployments). |
| מחרוזות בקוד מול מה שבאוויר | `rg` על `src/locales` ו־`src/pages` (עברית); אחר כך לאמת ב־bundle עם `BUNDLE_PROBES=…`. |
| מחרוזות שלא ב־locale | `rg '[\u0590-\u05FF]{4,}' src` (רועש — מסננים ידנית). |
| רגרסיה ויזואלית | Playwright עם `baseURL` לכל דומיין, או צילומי מסך ידניים לאותן נתיבים. |
